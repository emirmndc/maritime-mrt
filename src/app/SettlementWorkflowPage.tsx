...
const initialSettlement: SettlementItem = {
  ...
  evidenceRefs: ["Invoice", "Email", "PDA / FDA", "Recap"],
...
export function SettlementWorkflowPage() {
  ...
  const [selectedReason, setSelectedReason] = useState<(typeof reasonOptions)[number]>("Port cost difference");
  const [customReason, setCustomReason] = useState("");
  const [selectedEvidence, setSelectedEvidence] = useState<EvidenceRef[]>(["Invoice", "Email", "PDA / FDA"]);

  const resolvedReason = selectedReason === "Custom" ? customReason.trim() || "Custom review note" : selectedReason;
  ...
  const handleEvidenceToggle = (item: EvidenceRef) => {
  ...
  const handleSplitAndNeutralize = () => {
    ...
    setTimeline((current) => [
      {
        id: `evt-${Date.now()}-1`,
        action: "Tartismasiz kisim serbest birakildi",
        timestamp: createdAt,
        actor: "Settlement workflow",
      },
      {
        id: `evt-${Date.now()}-2`,
        action: "Tartismali kisim vault'a alindi",
        timestamp: createdAt,
        actor: "Settlement workflow",
      },
      {
        id: `evt-${Date.now()}-3`,
        action: "Karshi taraf yanit bekleniyor",
        timestamp: createdAt,
        actor: "Settlement workflow",
      },
      ...current,
  ...
  return (
    <AppShell
      ...
      description="Kucuk bir ihtilaf yuzunden tum odeme durmaz. Tartismasiz kisim akar. Sadece tartismali kisim izole edilir."
    >
        ...
        <Surface>
          ...
          <div className="mt-5 rounded-2xl border border-white/10 bg-black/10 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(vault?.evidenceRefs || settlement.evidenceRefs).map((item) => (
                <span
                  key={item}
            ...
            {timeline.map((event) => (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="font-semibold text-white/90">{event.action}</div>
                  <div className="text-xs text-[#88c4ff]">{event.timestamp}</div>
                </div>
...
function InfoCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
      ...
      <div className="mt-2 text-sm font-semibold leading-7 text-white/90">{value}</div>
    </div>
  );
}
...
function SplitNeutralizeModal({
  ...
  onEvidenceToggle: (value: EvidenceRef) => void;
  onConfirm: () => void;
}) {
  return (
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {evidenceOptions.map((item) => {
                const selected = selectedEvidence.includes(item);
                return (
                  <button
                    key={item}
                    type="button"
                    onClick={() => onEvidenceToggle(item)}
                    className={[
                      "rounded-2xl border px-4 py-3 text-left text-sm transition",
                      selected
